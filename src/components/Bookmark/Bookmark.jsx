...
const Bookmark = () => {
    ...
    const { loading, bookmark } = useBookmarks();

    if (loading) return <Loader />
    
    return (
        <div className="bookmarklist m-1 flex flex-col gap-y-2">
            <h2 className="font-bold text-md ml-1">Bookmarks ({bookmark.length})</h2>
            <div className="flex flex-col gap-y-3">
                {
                    bookmark.map(bk => (
                        <Link key={bk.id} to={`${bk.id}?lat=${bk.latitude}&lng=${bk.longitude}`}>
                            <div className="flex items-center gap-x-3 bg-slate-200 p-2 rounded-lg hover:bg-slate-100 ease-in-out duration-100">
